...
└── package.json    
  ...
  2. types 폴더:
    이 폴더는 애플리케이션에서 사용되는 TypeScript 타입과 인터페이스 정의를 포함합니다.
  3. utils 폴더:
    이 폴더는 애플리케이션 전반에 걸쳐 사용되는 유틸리티 함수들을 포함합니다.
  4.  pages 폴더:
    라우트에 직접 매핑되는 컴포넌트들을 포함합니다.
    주로 여러 컴포넌트들을 조합하여 전체 페이지를 구성합니다.
    URL과 1:1로 대응되는 경우가 많습니다.
  5. components:
    재사용 가능한 UI 요소들을 포함합니다.
    페이지나 다른 컴포넌트 내에서 사용됩니다.
    URL과 직접적인 관계가 없습니다.
    components 하위 폴더 구조:
    1. common: 애플리케이션 전반에 걸쳐 재사용되는 범용 컴포넌트들을 포함합니다.   
    2. feature: 특정 기능이나 도메인과 관련된 컴포넌트들을 포함합니다. 이들은 보통 특정 페이지나 기능에 종속적입니다.
    3. layout: 페이지의 구조를 정의하는 컴포넌트들을 포함합니다. 주로 여러 페이지에서 공통적으로 사용되는 레이아웃 요소들이 여기에 속합니다
  6. hooks 폴더:
    이 폴더는 재사용 가능한 커스텀 React 훅들을 포함합니다.
  7. styles 폴더
